...
// Refresh session every 4 minutes to prevent cookie expiry during a long shift
  ...
  if (!queryClient) {
    queryClient = new QueryClient({
      defaultOptions: {
        queries: {
          staleTime: 30 * 1000,
          // Booth app: aggressive retry — field conditions can have spotty connectivity
          retry: 3,
...
function buildAuthHeaders(): Record<string, string> {
  ...
  if (cookie) {
    // biome-ignore lint/complexity/useLiteralKeys: Headers index signature requires bracket notation
...
function AuthSessionKeepAlive() {
  useEffect(() => {
    const refresh = () => void ensureAuthCookiesFresh();
    refresh();
    const id = setInterval(refresh, SESSION_KEEPALIVE_MS);
...
export function TRPCReactProvider({ children }: { children: React.ReactNode }) {
  return (
    <QueryClientProvider client={getQueryClient()}>
      <TRPCProvider
        trpcClient={
          getTrpcClient() as unknown as Parameters<
            typeof TRPCProvider
          >[0]["trpcClient"]
        }
