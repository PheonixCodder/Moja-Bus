...
const toggleVariants = cva(
  cn(
    ...
    Platform.select({
      web: "hover:bg-muted hover:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 aria-invalid:border-destructive inline-flex cursor-default whitespace-nowrap outline-none transition-[color,box-shadow] focus-visible:ring-[3px] disabled:pointer-events-none [&_svg]:pointer-events-none",
    }),
  ),
  {
    variants: {
...
function ToggleIcon({
  ...
  return (
    <Icon
      className={cn("size-4 shrink-0", textClass as string, className)}
      {...props}
    />
  );
}
