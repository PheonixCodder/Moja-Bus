		...
		>
			<Pressable className="flex-1 bg-black/40" onPress={onClose}>
				<Pressable className="flex-1 justify-end" onPress={() => {}}>
					<View
						className="bg-card rounded-t-[28px] pt-5 px-4"
					...
					>
						...
						<View className="items-center gap-3">
							<View className="w-14 h-14 rounded-full bg-destructive/10 items-center justify-center">
								<HugeiconsIcon icon={Delete01Icon} size={28} color={Palette.rose[600]} />
							</View>
						...
						{/* Buttons */}
						<View className="flex-row gap-2 pt-5 mt-3 border-t border-border">
							<Pressable
								onPress={onClose}
								disabled={isPending}
							...
							<Pressable
								...
								disabled={isPending}
								accessibilityRole="button"
								className={`flex-1 py-2 rounded-xl bg-destructive items-center min-h-10 justify-center ${isPending ? "opacity-60" : ""}`}
							>
								{isPending ? (
