    ...
    >
      <View className="flex-row items-center justify-between mb-3.5">
            ...
            <View className="flex-row items-center mt-0.5 gap-2 flex-wrap">
              {offer.isExpress ? (
                <View className="flex-row items-center gap-1">
                  ...
                  <Text className="text-xs font-extrabold text-primary uppercase tracking-wide">
                    {t('expressNonStop')}
                  </Text>
                </View>
              ) : null}
              {offer.busTypeName ? (
                <Text className="text-xs font-semibold text-muted-foreground">{offer.busTypeName}</Text>
              ) : null}
            </View>
          </View>
        </View>
        ...
        <View className={`px-2.5 py-1 rounded-xl border ${badgeStyle.bgClass} ${badgeStyle.borderClass}`}>
          <Text className={`text-xs font-black uppercase tracking-wider ${badgeStyle.textClass}`}>
            {offer.busClass}
          </Text>
        </View>
      </View>
      ...
      <View className="bg-muted/50 rounded-2xl p-3 flex-row items-center border border-border mb-3">
        ...
        <View className="flex-1 items-center justify-center px-1.5">
          <Text className="text-xs font-bold text-muted-foreground mb-1">{offer.duration}</Text>
        ...
        <View className="flex-1 items-end">
          ...
          <Text className="text-xs font-medium text-muted-foreground mt-0.5 text-right" numberOfLines={1}>
          ...
          <View className="mt-1">
            {offer.availability === 'FEW_LEFT' ? (
              <View className="bg-warning/10 border border-warning/20 px-2 py-0.5 rounded-lg">
                <Text className="text-warning text-xs font-extrabold">
              ...
              <View className="bg-muted px-2 py-0.5 rounded-lg">
                <Text className="text-muted-foreground text-xs font-extrabold">{t('soldOut')}</Text>
              </View>
            ) : (
              <View className="bg-success/10 border border-success/20 px-2 py-0.5 rounded-lg">
                <Text className="text-success text-xs font-extrabold">
                  {typeof offer.remainingSeats === 'number'
