...
describe("sales-cutoff", () => {
  ...
  test("salesCutoffInstant adds exactly the cutoff to now", () => {
    ...
    assert.deepEqual(salesCutoffInstant(now), new Date("2026-08-02T14:30:00.000Z"));
  });
  ...
  test("isPastSalesCutoff: departure before the cutoff instant -> closed", () => {
    ...
    assert.equal(isPastSalesCutoff(new Date("2026-08-02T14:29:59.999Z"), now), true);
    assert.equal(isPastSalesCutoff(new Date("2026-08-02T09:00:00.000Z"), now), true);
  });

  test("isPastSalesCutoff: departure exactly at the cutoff -> still bookable", () => {
    const now = new Date("2026-08-02T14:00:00.000Z");
    assert.equal(isPastSalesCutoff(new Date("2026-08-02T14:30:00.000Z"), now), false);
  });

  test("isPastSalesCutoff: departure after the cutoff -> bookable", () => {
    const now = new Date("2026-08-02T14:00:00.000Z");
    assert.equal(isPastSalesCutoff(new Date("2026-08-02T16:00:00.000Z"), now), false);
  });
