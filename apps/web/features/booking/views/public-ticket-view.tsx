...
export function PublicTicketView({ ticketToken }: PublicTicketViewProps) {
  ...
  return (
    <div className="max-w-lg mx-auto px-4 py-8 space-y-6 print:py-0 print:px-0 print:max-w-full">
      <div className="rounded-xl border border-success/30 bg-success/10 px-4 py-3 flex flex-col sm:flex-row sm:items-center justify-between gap-3 print:hidden">
        <div className="flex gap-3 items-start">
          <ShieldCheck className="size-5 text-success shrink-0 mt-0.5" />
          <div className="text-sm text-success">
            <p className="font-semibold">{t("validTicket")}</p>
            <p className="text-success/90 mt-0.5 text-xs">
              {t("validTicketDesc")}
            </p>
          </div>
        </div>
      ...
      <p className="text-center text-xs text-muted-foreground print:text-foreground print:mt-4">
