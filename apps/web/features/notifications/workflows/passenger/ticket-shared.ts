  ...
  async ({ step, payload }) => {
    ...
    if (payload.phone) {
      await step.sms("send-sms", async () => ({
        body: `Moja Ride: ${payload.senderName} shared a bus ticket to ${payload.destinationCity} (${payload.departureTime}) with you! View ticket here: https://mojaride.com/tickets/${payload.ticketToken}`,
      }));
    }
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #ee237c; border-radius: 12px; padding: 24px; color: #1e293b; text-align: center;">
          ...
          <p><strong>${payload.senderName}</strong> has shared a digital bus ticket to <strong>${payload.destinationCity}</strong> with you.</p>
          <div style="background: #f8fafc; padding: 16px; border-radius: 8px; margin: 20px 0; font-size: 14px; text-align: left;">
            <p style="margin: 0 0 8px 0;">Route: <strong>${payload.originCity} to ${payload.destinationCity}</strong></p>
            <p style="margin: 0;">Departure: <strong>${payload.departureTime}</strong></p>
          </div>
          <a href="https://mojaride.com/tickets/${payload.ticketToken}" 
      ...
      return {
        subject: `${payload.senderName} shared a Moja Ride ticket with you!`,
        body: html,
      };
    });
  },
