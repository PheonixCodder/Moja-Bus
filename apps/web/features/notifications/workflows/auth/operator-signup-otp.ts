    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">Verify your email address to complete registration for <strong>${escapeHtml(payload.companyName)}</strong>:</p>
      ...
      return {
        subject: `${escapeHtml(payload.otpCode)} is your Moja Ride business verification code`,
        body: html,
      };
    });
  },
