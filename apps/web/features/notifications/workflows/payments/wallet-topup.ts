...
export const passengerWalletTopupWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">Hello ${payload.passengerName}, your wallet deposit has cleared.</p>
      ...
      return {
        subject: `Wallet Deposit Confirmation - ${payload.amountXOF} XOF`,
        body: html,
      };
    });
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Deposit of ${payload.amountXOF} XOF was successful! Funds are available immediately.`,
