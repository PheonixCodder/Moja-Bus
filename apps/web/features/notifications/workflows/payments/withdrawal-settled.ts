...
export const operatorWithdrawalSettledWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">The bank transfer for <strong>${payload.companyName}</strong> has cleared successfully:</p>
          <div style="background: #ecfdf5; border-left: 4px solid #10b981; padding: 16px; border-radius: 4px; margin: 16px 0; font-size: 14px; color: #065f46;">
            ...
            <p style="margin: 0 0 8px 0;">Bank: <strong>${payload.bankName} (****${payload.accountNumberLast4})</strong></p>
            <p style="margin: 0;">Settled At: <strong>${payload.settledAt}</strong></p>
          </div>
        </div>
      `;

      return {
        subject: `Withdrawal settled successfully: ${payload.amountXOF} XOF`,
        body: html,
      };
    });
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Withdrawal of ${payload.amountXOF} XOF to bank has cleared successfully.`,
    ...
    if (payload.phone) {
      await step.sms("send-sms", async () => ({
        body: `Moja Ride Payout: Payout of ${payload.amountXOF} XOF to your verified bank account has settled successfully.`,
      }));
    }
  },
  {
