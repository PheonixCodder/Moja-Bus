...
export const operatorWithdrawalRequestedWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">We have initiated a payout request for <strong>${payload.companyName}</strong> to your default bank account:</p>
      ...
      return {
        subject: `Withdrawal request initiated: ${payload.amountXOF} XOF`,
        body: html,
      };
    });
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Withdrawal request for ${payload.amountXOF} XOF sent to ${payload.bankName}.`,
