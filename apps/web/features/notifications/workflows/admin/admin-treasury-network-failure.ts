...
export const adminTreasuryNetworkFailureWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #fca5a5; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <div style="background: #fef2f2; border-left: 4px solid #ef4444; padding: 16px; border-radius: 4px; margin: 16px 0; font-size: 14px; color: #991b1b;">
            <p style="margin: 0 0 8px 0;">Company ID: <strong>${escapeHtml(payload.companyId)}</strong></p>
            <p style="margin: 0 0 8px 0;">Amount: <strong>${escapeHtml(payload.amountXOF)} XOF</strong></p>
            <p style="margin: 0 0 8px 0;">Ledger Transaction ID: <strong>${escapeHtml(payload.transactionId)}</strong></p>
            <p style="margin: 0;">Error Details: "<strong>${escapeHtml(payload.reason)}</strong>"</p>
          </div>
      ...
      return {
        subject: `🚨 CRITICAL: Payout Network Failure - ${escapeHtml(payload.amountXOF)} XOF`,
        body: html,
      };
    });
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Ledger Tx ${escapeHtml(payload.transactionId)} posted but Paystack API failed: ${escapeHtml(payload.reason)}.`,
    ...
    await step.chat("send-slack", async () => ({
      body: `🚨 *Treasury Payout Network Failure* 🚨\nCompany ID: *${escapeHtml(payload.companyId)}*\nAmount: *${escapeHtml(payload.amountXOF)} XOF*\nLedger TxID: \`${escapeHtml(payload.transactionId)}\`\nReason: *${escapeHtml(payload.reason)}*`,
    }));
  },
  {
