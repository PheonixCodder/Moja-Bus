...
export const operatorAccountRestoredWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `🎉 Welcome back! ${escapeHtml(payload.companyName)} account is reactivated. Routes and scheduling are online.`,
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #10b981; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p>We are pleased to inform you that your company account <strong>${escapeHtml(payload.companyName)}</strong> has been reactivated and fully restored by Moja Ride administration.</p>
