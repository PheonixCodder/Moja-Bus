  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #dc2626; border-radius: 12px; padding: 24px; color: #7f1d1d; background: #fef2f2;">
          ...
          <p>Hello ${payload.operatorName},</p>
          <p>We are writing to notify you that the transport company account for <strong>${payload.companyName}</strong> has been suspended by Moja Ride administration.</p>
    ...
    if (payload.phone) {
      await step.sms("send-sms", async () => ({
        body: `Moja Ride: Account for ${payload.companyName} has been suspended. All scheduling and sales are offline. Contact support.`,
      }));
    }
  },
  {
