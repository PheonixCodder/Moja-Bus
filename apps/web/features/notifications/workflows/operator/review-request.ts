...
export const passengerReviewRequestWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 480px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b; text-align: center;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">Thank you for riding with <strong>${payload.companyName}</strong> on your trip from <strong>${payload.originCity} to ${payload.destinationCity}</strong>.</p>
      ...
      return {
        subject: `How was your trip with ${payload.companyName}?`,
        body: html,
      };
    });
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Welcome back! Rate your recent journey from ${payload.originCity} to ${payload.destinationCity} with ${payload.companyName}.`,
