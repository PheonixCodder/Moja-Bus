...
export const passengerTripCancelledWorkflow = workflow(
  ...
  async ({ step, payload }) => {
    ...
    await step.email("send-email", async () => {
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 500px; margin: 0 auto; border: 1px solid #fca5a5; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">We regret to inform you that your upcoming trip from <strong>${payload.originCity} to ${payload.destinationCity}</strong> scheduled for <strong>${payload.departureTime}</strong> has been cancelled by the transport operator.</p>
    ...
    await step.inApp("send-in-app", async () => ({
      ...
      body: `Trip to ${payload.destinationCity} (${payload.departureTime}) was CANCELLED: ${payload.cancelReason}. Balance refunded.`,
    ...
    if (payload.phone) {
      await step.sms("send-sms", async () => ({
        body: `Moja Ride URGENT: Trip from ${payload.originCity} to ${payload.destinationCity} (${payload.departureTime}) has been CANCELLED due to: ${payload.cancelReason}. ${payload.refundAmountXOF} XOF has been refunded to your wallet.`,
      }));
    }
  },
  {
