      ...
      const html = `
        <div style="font-family: Arial, sans-serif; max-width: 520px; margin: 0 auto; border: 1px solid #e2e8f0; border-radius: 12px; padding: 24px; color: #1e293b;">
          ...
          <p style="font-size: 15px; line-height: 1.5; color: #334155;">
            <strong>${escapeHtml(payload.inviterName)}</strong> has invited you to join <strong>${escapeHtml(payload.companyName)}</strong> as <strong>${escapeHtml(payload.role)}</strong>.
          </p>
          ${messageBlock}
          ...
          <p style="font-size: 12px; color: #64748b; margin-top: 24px;">
            ...
            <a href="${escapeHtml(payload.inviteUrl)}" style="color: #ee237c; word-break: break-all;">${escapeHtml(payload.inviteUrl)}</a>
          </p>
        </div>
      `;

      return {
        subject: `${escapeHtml(payload.inviterName)} invited you to join ${escapeHtml(payload.companyName)} on Moja Ride`,
        body: html,
      };
    });
  },
