  ...
  const checklist = [
    ...
    },
    {
      label: "Required documents approved",
        ...
        getDocumentsVerificationState(settings?.company.documents || []) ===
        "approved",
    },
  ];
        ...
        <div className="flex-1 space-y-2">
          {checklist.map((item, idx) => (
            <div key={idx} className="flex items-center gap-3 text-sm">
              <div
                className={`w-4 h-4 rounded-full flex items-center justify-center shrink-0 ${item.done ? "bg-primary text-primary-foreground" : "bg-muted border border-input"}`}
              >
                {item.done && (
                  <svg
                    viewBox="0 0 24 24"
