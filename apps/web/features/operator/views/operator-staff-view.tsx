...
const ROLE_PERMISSION_PREVIEW: Record<StaffRole, string[]> = {
  OWNER: [
    "Invite Staff", "Manage Staff", "View Staff", "Manage Routes",
    "Manage Schedules", "Manage Fleet", "View Finance", "Manage Finance",
    "Manage Dispatch", "View Trips", "View Bookings", "Transfer Ownership",
  ],
  ADMIN: [
    "Invite Staff", "Manage Staff", "View Staff", "Manage Routes",
    "Manage Schedules", "Manage Fleet", "View Finance", "Manage Dispatch",
  ],
  MANAGER: ["Manage Routes", "Manage Schedules", "Manage Dispatch", "View Staff"],
...
const INVITABLE_ROLES: StaffRole[] = ["ADMIN", "MANAGER", "OPERATIONS", "FINANCE", "SUPPORT"];
...
function MemberAvatar({ name, size = "md" }: { name: string; size?: "sm" | "md" | "lg" }) {
  ...
  const sizeClass = size === "sm" ? "h-8 w-8 text-[11px]" : size === "lg" ? "h-12 w-12 text-base" : "h-10 w-10 text-[13px]";
  return (
    <div className={cn("flex shrink-0 items-center justify-center rounded-full font-semibold text-white", sizeClass, color)}>
      {initials}
    </div>
  );
}
...
function RoleBadge({ role }: { role: StaffRole }) {
  return (
    <span className={cn("inline-flex items-center rounded-md border px-2 py-0.5 text-[11px] font-semibold", ROLE_COLORS[role])}>
      {ROLE_LABELS[role]}
    </span>
  );
}
...
function StatusBadge({ status }: { status: OperatorStatus }) {
  ...
  return (
    <span className={cn("inline-flex items-center gap-1 text-[12px] font-medium", config.className)}>
...
function RoleSheet({ member, open, onClose, onSave, callerRole }: RoleSheetProps) {
  const [selectedRole, setSelectedRole] = useState<StaffRole>(member?.role ?? "OPERATIONS");
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => !v && onClose()}>
      <SheetContent side="right" className="flex w-full flex-col max-w-sm sm:max-w-md border-l border-border bg-card p-6 overflow-y-auto">
          ...
          {member && (
            <SheetDescription className="flex items-center gap-3 mt-2 rounded-lg bg-accent/50 p-3">
              ...
              <div className="flex flex-col text-left">
                <span className="text-[13px] font-medium text-foreground">{member.user.fullName}</span>
        ...
        <div className="flex-1 space-y-8">
          <div className="space-y-3">
            <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Current Role</Label>
            <div>
          ...
          <div className="space-y-3">
            <Label htmlFor="role-select" className="text-xs font-medium text-muted-foreground uppercase tracking-wider">New Role</Label>
            <Select value={selectedRole} onValueChange={(v) => setSelectedRole(v as StaffRole)}>
              <SelectTrigger id="role-select" className="h-10 border-border bg-background text-[13px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-3">
            <Label className="text-xs font-medium text-muted-foreground uppercase tracking-wider">Permissions Preview</Label>
            <div className="rounded-xl border border-border bg-background p-4 space-y-2.5 shadow-sm">
              {ALL_PERMISSIONS.map((perm) => {
                const has = allowed.includes(perm);
                return (
                  <div key={perm} className={cn("flex items-center gap-3 text-[13px]", has ? "text-foreground font-medium" : "text-muted-foreground/60")}>
                    <span className={cn("flex items-center justify-center h-4 w-4 rounded-full text-[10px] font-bold", has ? "bg-emerald-500/15 text-emerald-600" : "bg-muted text-muted-foreground/40")}>
                      {has ? "✓" : "✗"}
                    </span>
                    {perm}
                  </div>
...
function InviteSheet({ open, onClose, onSend }: InviteSheetProps) {
  const [form, setForm] = useState({ email: "", role: "OPERATIONS" as StaffRole, jobTitle: "", message: "" });
  ...
  async function handleSend() {
    ...
    setSending(true);
    try {
      const payload: CreateInvitationPayload = { email: form.email, role: form.role };
  ...
  return (
    <Sheet open={open} onOpenChange={(v) => { if (!v) { reset(); onClose(); } }}>
      <SheetContent side="right" className="flex w-full flex-col max-w-sm sm:max-w-md border-l border-border bg-card p-6 overflow-y-auto">
        <SheetHeader className="mb-8">
          <SheetTitle className="text-lg font-semibold">Invite Team Member</SheetTitle>
        ...
        <div className="flex-1 space-y-6">
          <div className="space-y-2">
            <Label htmlFor="invite-email" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Email *</Label>
            <Input
              id="invite-email"
              type="email"
          ...
          <div className="space-y-2">
            <Label htmlFor="invite-role" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">Role *</Label>
            <Select value={form.role} onValueChange={(v) => setForm((f) => ({ ...f, role: v as StaffRole }))}>
              <SelectTrigger id="invite-role" className="h-10 border-border text-[13px]">
                <SelectValue />
              </SelectTrigger>
              <SelectContent>
                {INVITABLE_ROLES.map((r) => (
          ...
          <div className="space-y-2">
            <Label htmlFor="invite-title" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
              Job Title <span className="text-muted-foreground/60 normal-case tracking-normal">(optional)</span>
            </Label>
            <Input
              id="invite-title"
          ...
          <div className="space-y-2">
            <Label htmlFor="invite-message" className="text-xs font-medium uppercase tracking-wider text-muted-foreground">
              Welcome Message <span className="text-muted-foreground/60 normal-case tracking-normal">(optional)</span>
            </Label>
            <Textarea
              id="invite-message"
            ...
            />
            <p className="text-[11px] text-muted-foreground mt-1.5">Shown on the invite acceptance screen.</p>
          </div>
        </div>

        <div className="mt-8 pt-4 border-t border-border flex gap-3">
          <Button variant="outline" className="flex-1 h-10 text-[13px]" onClick={() => { reset(); onClose(); }}>
            Cancel
          </Button>
          <Button
            className="flex-1 h-10 text-[13px] bg-[#ee237c] hover:bg-[#d11f6e] text-white"
            ...
            disabled={sending}
          >
            {sending ? <Spinner className="h-4 w-4" /> : <><Send className="mr-2 h-4 w-4" />Send Invite</>}
          </Button>
        </div>
      </SheetContent>
    </Sheet>
...
function TransferOwnershipDialog({ member, open, onClose, onConfirm }: TransferDialogProps) {
  ...
  return (
    <AlertDialog open={open} onOpenChange={(v) => { if (!v) { setPassword(""); onClose(); } }}>
      <AlertDialogContent className="border-border bg-card max-w-md">
        <AlertDialogHeader>
          <div className="flex items-center gap-2 mb-1">
            <AlertTriangle className="h-5 w-5 text-amber-500" />
            <AlertDialogTitle className="text-base font-semibold">Transfer Ownership</AlertDialogTitle>
          </div>
        ...
        <div className="space-y-1.5 px-0 pb-2">
          <Label htmlFor="transfer-password" className="text-[12px] font-medium text-muted-foreground">
        ...
        <AlertDialogFooter>
          <AlertDialogCancel className="h-9 text-[13px]" onClick={() => setPassword("")}>
            Cancel
          </AlertDialogCancel>
          <AlertDialogAction
          ...
          >
            {confirming ? <Spinner className="h-3.5 w-3.5" /> : "Transfer Ownership"}
          </AlertDialogAction>
        </AlertDialogFooter>
      </AlertDialogContent>
    </AlertDialog>
...
export function OperatorStaffView() {
  ...
  const [roleSheetMember, setRoleSheetMember] = useState<StaffMember | null>(null);
  const [transferMember, setTransferMember] = useState<StaffMember | null>(null);
  const [statusMember, setStatusMember] = useState<{ member: StaffMember; nextStatus: OperatorStatus } | null>(null);

  // Determine caller's role from the roster (first OWNER or ADMIN = caller)
  // In a real app this would come from session context. We'll infer from API response.
  const callerRole: StaffRole = "OWNER"; // TODO: wire from session context
  ...
  async function handleStatusChange(member: StaffMember, nextStatus: OperatorStatus) {
    try {
      await updateStaffStatus(member.id, nextStatus);
      toast.success(`${member.user.fullName} is now ${STATUS_CONFIG[nextStatus].label}`);
      await fetchAll();
    } catch (err: any) {
  ...
  async function handleTransfer(memberId: string, password: string) {
    try {
      const res = await transferOwnership(memberId, password);
  ...
  function groupActivityByDate(logs: ActivityLogEntry[]) {
    ...
    for (const log of logs) {
      ...
      const label = d === today ? "Today" : d === yesterday ? "Yesterday" : format(new Date(log.createdAt), "MMM d");
  ...
  if (loading) {
    return (
      <div className="flex h-96 items-center justify-center">
        <Spinner className="h-6 w-6 text-[#ee237c]" />
      </div>
    );
  }
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0 bg-bg-base">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-border bg-card px-6 py-3.5 gap-3.5 shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-2">
            ...
            <Select value={statusFilter} onValueChange={(v) => setStatusFilter(v as OperatorStatus | "ALL")}>
              <SelectTrigger className="h-8.5 w-[130px] border-border bg-bg-base text-xs">
        ...
        <section>
          ...
          {members.length === 0 ? (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-border bg-card py-16 text-center shadow-sm">
              ...
              <p className="text-[14px] font-medium text-foreground">No team members yet</p>
                  ...
                  >
                    <div className="flex items-start justify-between gap-2">
                      <div className="flex flex-col min-w-0">
                        ...
                        <div className="flex items-center gap-1.5 mt-1">
                          ...
                          {inv.jobTitle && (
                            <span className="text-[11px] text-muted-foreground">{inv.jobTitle}</span>
                          )}
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="space-y-0.5">
                      <div className="flex items-center gap-1.5 text-[11px] text-muted-foreground">
                        ...
                        <span>Sent by {inv.invitedBy.fullName} · {format(new Date(inv.createdAt), "MMM d, HH:mm")}</span>
                      </div>
                      <div className={cn("flex items-center gap-1.5 text-[11px] font-medium", expiry.expired ? "text-red-500" : "text-amber-600")}>
                  ...
                  <div className="divide-y divide-border/50">
                    {logs.map((log) => (
                      <div key={log.id} className="flex items-start gap-3 px-4 py-3 hover:bg-accent/20 transition-colors">
                        ...
                        <div className="flex min-w-0 flex-1 flex-col">
                          <span className="text-[13px] text-foreground">{log.description}</span>
                        </div>
      ...
      {/* ── Sheets & Dialogs ── */}
      <InviteSheet open={inviteOpen} onClose={() => setInviteOpen(false)} onSend={handleInvite} />
