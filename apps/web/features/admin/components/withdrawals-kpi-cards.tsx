    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border border-border rounded-xl bg-bg-base/50 p-1">
      <div className="flex flex-col gap-1 p-5 border border-border/50 rounded-lg bg-bg-base shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-medium text-sm text-text-muted">{t("pendingQueue")}</h3>
        </div>
        <p className="text-2xl font-bold text-text-primary tracking-tight">
          {stats.pendingCount} <span className="text-sm font-normal text-text-muted">{t("requests")}</span>
        </p>
      ...
      <div className="flex flex-col gap-1 p-5 border border-border/50 rounded-lg bg-bg-base shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-medium text-sm text-text-muted">{t("totalSettled")}</h3>
        </div>
        <p className="text-2xl font-bold text-text-primary tracking-tight">
          {stats.settledCount} <span className="text-sm font-normal text-text-muted">{t("payouts")}</span>
        </p>
      ...
      <div className="flex flex-col gap-1 p-5 border border-border/50 rounded-lg bg-bg-base shadow-sm">
        <div className="flex items-center gap-2 mb-2">
          ...
          <h3 className="font-medium text-sm text-text-muted">{t("failedReversed")}</h3>
        </div>
        <p className="text-2xl font-bold text-text-primary tracking-tight">
          {stats.failedCount} <span className="text-sm font-normal text-text-muted">{t("reversed")}</span>
        </p>
      ...
      <div className="col-span-1 sm:col-span-3 px-4 py-2 mt-1 flex items-center justify-between text-xs text-text-muted">
        <div className="flex items-center gap-1.5">
          ...
          <span>{t("showingData")} <span className="font-semibold text-text-primary">{stats.totalCount}</span></span>
        </div>
      </div>
    </div>
  );
