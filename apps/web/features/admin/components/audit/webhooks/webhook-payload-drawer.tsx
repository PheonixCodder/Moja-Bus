    ...
    <Sheet open={!!event} onOpenChange={(open) => !open && onClose()}>
      <SheetContent className="sm:max-w-xl overflow-y-auto">
        <SheetHeader className="mb-6">
          ...
          <SheetDescription>
            {t("receivedOn", { date: format(new Date(event.createdAt), "MMM d, yyyy 'at' h:mm a") })}
          </SheetDescription>
        </SheetHeader>
        ...
        <div className="space-y-6">
          <div className="grid grid-cols-2 gap-4 text-sm">
            <div>
              <span className="text-muted-foreground block mb-1">{t("provider")}</span>
              <span className="font-medium capitalize">{event.provider}</span>
            </div>
            <div>
              <span className="text-muted-foreground block mb-1">{t("eventType")}</span>
              <span className="font-medium font-mono bg-muted px-1 rounded">{event.eventType}</span>
            </div>
            <div className="col-span-2">
              <span className="text-muted-foreground block mb-1">{t("idempotencyKey")}</span>
            ...
            {event.reference && (
              <div className="col-span-2">
                <span className="text-muted-foreground block mb-1">{t("reference")}</span>
                <span className="font-medium">{event.reference}</span>
              </div>
            )}
            {event.processedAt && (
              <div className="col-span-2">
                <span className="text-muted-foreground block mb-1">{t("processedAt")}</span>
          ...
          {event.error && (
            <div>
              <h4 className="text-sm font-semibold mb-2 text-red-500">{t("processingError")}</h4>
