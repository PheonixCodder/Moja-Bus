...
export class PaymentService {
  ...
  async initiateForHold(
    ...
    const effectiveProvider =
      providerId === "PAYSTACK" && !isPaystackConfigured()
        ? "MOCK"
        : providerId;

    if (effectiveProvider === "MOCK") {
      return this.initiateMock(holdGroup.id, snapshot.chargeAmountXOF, snapshot);
    }

    if (effectiveProvider !== "PAYSTACK") {
      throw new TRPCError({
        code: "BAD_REQUEST",
        message: `${providerId} is not available yet. Use card/mobile money via Paystack.`,
      });
    }
    ...
    const payment = await this.prisma.$transaction(async (tx) => {
      ...
      await tx.paymentAttempt.create({
        data: {
          paymentId: record.id,
          attemptNumber: 1,
          paystackReference: `mock_${holdGroupId}`,
          status: "SUCCESS",
        },
      });
  ...
  async verifyAndConfirm(
    reference: string,
    userId?: string | null,
  ): Promise<{ confirmed: boolean; holdGroupId: string }> {
    ...
    if (payment.status === "SUCCESS") {
      ...
      return { confirmed: true, holdGroupId: payment.holdGroupId };
    }
