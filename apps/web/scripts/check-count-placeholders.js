...
console.log('=== EN keys with {count} placeholder (' + enCountResults.length + ') ===');
enCountResults.forEach(r => console.log('  ' + r.path + ': ' + JSON.stringify(r.en)));

console.log('\n=== Keys in EN but NOT in FR (' + [...enPaths].filter(p => !frPaths.has(p)).length + ') ===');
[...enPaths].filter(p => !frPaths.has(p)).forEach(p => console.log('  ' + p));

console.log('\n=== Keys in FR but NOT in EN (' + [...frPaths].filter(p => !enPaths.has(p)).length + ') ===');
[...frPaths].filter(p => !enPaths.has(p)).forEach(p => console.log('  ' + p));

console.log('\n=== EN keys with ICU plural ({count, plural}) (' + enPluralResults.length + ') ===');
enPluralResults.forEach(r => console.log('  ' + r.path + ': ' + JSON.stringify(r.en)));
