...
export default async function PassengerAuthLayout({
  children,
}: {
  children: ReactNode;
}) {
  const t = await getTranslations("auth.passenger.cover");
  ...
  return (
    <main className="min-h-screen bg-bg-surface">
      ...
      <div className="grid min-h-screen justify-center p-2 lg:grid-cols-2">
        ...
        {/* Left Form Column */}
        <div className="relative order-1 flex h-full w-full items-center justify-center">
          {children}
        </div>
      </div>
    </main>
