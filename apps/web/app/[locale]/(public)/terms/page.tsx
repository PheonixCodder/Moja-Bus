          ...
          <aside className="hidden lg:block">
            <div className="sticky top-8">
              ...
              <div className="mt-8 p-4 bg-slate-50 rounded-2xl border border-slate-100">
                <p className="text-xs font-semibold text-slate-600 mb-1">{t("questions")}</p>
                <a
          ...
          <div className="space-y-14 text-slate-600 leading-relaxed text-sm min-w-0">
            {data.items.map((item) => (
              <section key={item.id} id={item.id} className="scroll-mt-8">
                ...
                {item.subsections && (
                  <div className={item.id === "payment" ? "space-y-6" : "space-y-4"}>
                    {item.subsections.map((sub, si) => (
                      <div key={si}>
                        {sub.heading && (
                          <p className="font-semibold text-slate-700 mb-2">{sub.heading}</p>
                        )}
                        {sub.body?.map((p, pi) => (
                          <p key={pi} className={pi > 0 && si > 0 ? "mt-3" : item.id === "cancellation-modification" && pi > 0 ? "mt-3" : "mb-2"}>{p}</p>
                        ))}
                    ...
                    {item.id === "cancellation-modification" && item.paragraphs && (
                      <div className="space-y-3">
                        {item.paragraphs.map((p, i) => (
                          <p key={i}>{p}</p>
                        ))}
                ...
                {item.noteBox && (
                  <div className={item.id === "cancellation-policy" ? "bg-slate-50 border border-slate-200 rounded-2xl p-5 mb-6 text-slate-600 text-sm space-y-3" : "bg-pink-50 border border-pink-100 rounded-2xl p-5 mb-4"}>
                ...
                {item.id === "cancellation-policy" && item.subsections && (
                  <>
                    {item.subsections.map((sub, si) => (
                      <div key={si}>
                        {sub.body?.map((p, pi) => (
                          <p key={pi} className="text-xs text-slate-400 mb-6">{p}</p>
                        ))}
                      </div>
                    ))}
                  </>
                      ...
                      <table className="w-full text-sm">
                        <thead>
                          <tr className="bg-slate-900 text-white">
                            <th className="text-left px-5 py-3 font-semibold">{item.table.headers[0]}</th>
                            <th className="text-right px-5 py-3 font-semibold">{item.table.headers[1]}</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-100">
                          {item.table.rows.map((row, i) => (
                            <tr key={row[0]} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                              <td className="px-5 py-3 text-slate-700">{row[0]}</td>
                              <td className={`px-5 py-3 text-right font-bold ${item.table?.refundColor ?? "text-slate-600"}`}>{row[1]}</td>
                            </tr>
                          ))}
                        </tbody>
                      </table>
                          ...
                          <table className="w-full text-sm">
                            <thead>
                              <tr className="bg-slate-800 text-white">
                                <th className="text-left px-5 py-3 font-semibold">{item.olderPolicy.table.headers[0]}</th>
                                <th className="text-right px-5 py-3 font-semibold">{item.olderPolicy.table.headers[1]}</th>
                              </tr>
                            </thead>
                            <tbody className="divide-y divide-slate-100">
                              {item.olderPolicy.table.rows.map((row, i) => (
                                <tr key={row[0]} className={i % 2 === 0 ? "bg-white" : "bg-slate-50"}>
                                  <td className="px-5 py-3 text-slate-700">{row[0]}</td>
                                  <td className="px-5 py-3 text-right font-bold text-slate-600">{row[1]}</td>
                                </tr>
                              ))}
                            </tbody>
                          </table>
                        </div>
                        <p className="mt-3 text-xs text-slate-500">{item.olderPolicy.extras}</p>
                      </div>
                    )}
                  </>
                )}
                ...
                {item.contactBox && (
                  <div className="bg-slate-50 rounded-2xl p-6 border border-slate-100">
                    <p className="font-semibold text-slate-800">{item.contactBox.name}</p>
                    <p className="text-slate-500 mt-1">{item.contactBox.address}</p>
                    <a
