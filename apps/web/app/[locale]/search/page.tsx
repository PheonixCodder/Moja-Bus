...
export default async function SearchPage({ searchParams }: SearchPageProps) {
  ...
  if (params.from) {
    await prefetch(
      trpc.locations.getCityDetails.queryOptions({ id: params.from }),
    );
  }
  if (params.to) {
    await prefetch(
      trpc.locations.getCityDetails.queryOptions({ id: params.to }),
    );
  }
  if (params.from && params.to && params.date) {
    await prefetch(
      trpc.search.search.queryOptions({
        ...
        departureTime: params.departureTime?.length
