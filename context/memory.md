...
## Completed Work
- **Schedule + Search audit fixes — Task 3 of 9: `schedules.delete` soft-archives trips (DONE, 2026-08-08):**
  - **Bug (R7b):** `Booking.trip` has no `onDelete` → Prisma default `Restrict`, so `schedules.delete`'s `trip.deleteMany({ scheduleId })` crashed with an FK error whenever ANY trip had a booking row (even COMPLETED/CANCELLED history). Tasks 1-2 (commits `60040af`, `0a46265`) made `Trip.scheduleId` nullable with `onDelete: SetNull` + added `Trip.archivedAt DateTime?` and fixed null-safety ripples.
  - **Fix (`apps/web/trpc/routers/schedules.ts` delete, ~734-767):** inside the transaction, trips with `_count.bookings === 0` are hard-deleted; trips with any booking rows get `scheduleId: null` + `archivedAt: new Date()` (booking/financial history survives); then the schedule row is deleted. Existing CONFIRMED/PENDING_PAYMENT guard unchanged.
  - **Hidden archived trips (`archivedAt: null`) at:** `trips.list` (filters), `trips.statusCounts`, `trips.get`, `trips.getManifest`, `trips.getSeatMap`, `operator.getDashboardMetrics` todayTrips, `operator.globalSearch` trips (defense-in-depth — already safe via `schedule:{...}` relation filter), `admin.listOperations`, `admin.listDispatchTrips`, `admin.getDispatchTrip` (findUnique→findFirst), `admin.getTripAudit` (findUnique→findFirst). Left alone deliberately: `trips.create` conflict check + all operator trip mutations (by-id, not listings, reachable only from listings we now filter), booking-creation/notification lookups (`booking.ts:58` hold-notif, `payments.ts:35` checkout pricing, `fleet.ts:307` deleteBus guard), `schedules.ts` internal `scheduleId`-keyed queries (99/179/785/1396 — archived trips have null scheduleId so can never match), and public search (already filters `schedule: { isActive: true }`).
  - **Invariant preserved:** `archivedAt == null ⟹ trip has a schedule`; archived trips are invisible to operators/admins/public while their bookings stay searchable.
  - **Verification:** `pnpm typecheck --filter web` 0 errors; biome on the 4 touched router files = identical 7 errors/75 warnings before and after (all pre-existing; zero new diagnostics on changed lines). Pre-commit hook ran full turbo suite — **web 272/272 + schemas 27/27, all 8 tasks green**. Commit `4a72ec2`.
