...
| `/(tabs)/live` | `features/live/screens/live-view.tsx` | Real-time in-flight navigation HUD. Renders Mapbox turn-by-turn route, live speedometer gauge, stop arrival/departure buttons, delay report modal, emergency vehicle-breakdown trigger, mandated rest-break logging (`logRestBreak`), and a **"Take Wheel" handover button** that transfers active driving control + telemetry streaming to an assigned Relief driver. | `drivers.getMyProfile`, `drivers.recordStopArrival`, `drivers.recordStopDeparture`, `drivers.reportTripDelay`, `drivers.reportVehicleBreakdown`, `drivers.logRestBreak`, `drivers.handoverTripControl`, `drivers.completeTrip` |
...
| `/trip/[id]/manifest` | `features/trips/screens/manifest-view.tsx` | Passenger manifest for a departure. Lists seat numbers, passenger phone numbers (with one-tap dialer), boarding status, manual check-in triggers, and a **QR Scanner launcher** usable on `SCHEDULED`/`BOARDING`/`DELAYED` trips — enabling conductor gate boarding before the bus departs. | `drivers.getMyTripManifest`, `drivers.manualCheckInPassenger` |
