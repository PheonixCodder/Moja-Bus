...
## 1. File
Exact source path: [`apps/booth-app/components/ui/input.tsx`](file:///C:/dev/moja-buss/apps/booth-app/components/ui/input.tsx)
...
## 3. Purpose
Text input primitive enforcing 48px minimum touch height, left/right icon slots, error text, and PlaceholderColor.

## 4. Responsibilities
- Provide a reusable, theme-aware `input` UI primitive for the Booth application.
...
1. **Is it used?**: Yes.
2. **Where is it used?**: app/(tabs)/checkin.tsx, app/(tabs)/index.tsx, app/sell/passenger.tsx, app/terminal-select.tsx.
...
- **Decision**: `KEEP`
- **Reason**: Keep as canonical text input.
