...
## 1. File
Exact source path: [`apps/booth-app/components/ui/button.tsx`](file:///C:/dev/moja-buss/apps/booth-app/components/ui/button.tsx)
...
## 3. Purpose
Primary touchable button primitive enforcing 48px minimum height, tactile haptic feedback, loading spinner, and semantic variants.

## 4. Responsibilities
- Provide a reusable, theme-aware `button` UI primitive for the Booth application.
...
- **Active Usage**: YES (8 consumers)
- **Over-Engineered**: NO
- **Belongs in Booth App**: YES

## 8. UI / UX Audit
- **Touch Target**: Strictly enforces >= 48px min height.
...
1. **Is it used?**: Yes.
2. **Where is it used?**: app/(tabs)/checkin.tsx, app/(tabs)/profile.tsx, app/reconcile.tsx, app/sell/confirmation.tsx, app/sell/passenger.tsx, app/sell/[tripId].tsx, app/terminal-select.tsx, components/paystack-qr.tsx.
...
Keep as canonical button across all cashier screens.
...
- **Decision**: `KEEP`
- **Reason**: Keep as canonical button across all cashier screens.
